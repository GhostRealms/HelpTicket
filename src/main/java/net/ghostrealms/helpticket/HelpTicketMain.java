...
import net.ghostrealms.helpticket.cmds.TicketCommandHandler;
import net.ghostrealms.helpticket.events.HelpTicketListener;
...
public class HelpTicketMain extends DeityPlugin {
    ...
    protected void initConfig() {
        this.config.addDefaultConfigValue(HelpTicketConfigHelper.MAX_TICKETS_OPEN, 1);
    }
    ...
    protected void initLanguage() {
        ...
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_CREATE_STAFF,
                "%ticket_owner% has opened a new ticket &6%ticket_info%");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_CREATE_FAIL,
                "Instead of resubmitting that ticket. Please use &6/ticket info %ticket_id%");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_CREATE_MAX, "You can only have 1 ticket open at a time");
        ...
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_INFO_FAIL_SESSION_INVALID,
                "Before you can use this command you first must &c/ticket select [ticket-id]");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_INFO_FAIL_TICKET_INVALID,
                "Ticket %ticket_id% does not exist");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_PRIORITY_FAIL_TOO_HIGH,
                "This ticket already has the maximum priority");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_PRIORITY_FAIL_TOO_LOW,
                "This ticket already has the lowest priority");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_PRIORITY_SUCCESS,
                "Ticket %ticket_id%'s priority was changed to %ticket_priority%");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_ASSIGN_SUCCESS,
                "Ticket %ticket_id% was assigned to %ticket_assignee%");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_CLOSE_SUCCESS, "Ticket %ticket_id% was closed");
        this.language.addDefaultLanguageValue(HelpTicketLanguageHelper.TICKET_COMMENT_SUCCESS,
                "Ticket %ticket_id% had a comment added to it");
    ...
    protected void initDatabase() {
        DeityAPI.getAPI()
                .getDataAPI()
                .getMySQL()
                .write("CREATE TABLE IF NOT EXISTS " + getTicketsTableName() + " (" + "`id` INT( 16 ) NOT NULL AUTO_INCREMENT, "
                        + "`owner` VARCHAR(20) NOT NULL, " + "`location_id` INT( 16 ) NOT NULL, " + "`info` TEXT NOT NULL, "
                        + "`assignee` VARCHAR(40) NULL DEFAULT NULL, " + "`priority` VARCHAR(8) NOT NULL DEFAULT 'MEDIUM', "
                        + "`read_status` VARCHAR(8) NOT NULL DEFAULT 'READ', " + "`open_status` VARCHAR(8) NOT NULL DEFAULT 'OPEN', "
                        + "`creation_date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, " + "PRIMARY KEY (`id`),"
                        + "INDEX (`owner`))" + " ENGINE = MYISAM;", new Object[0]);
        
        DeityAPI.getAPI()
                .getDataAPI()
                .getMySQL()
                .write("CREATE TABLE IF NOT EXISTS " + getTicketLocationsTableName() + " ("
        ...
        DeityAPI.getAPI()
                .getDataAPI()
                .getMySQL()
                .write("CREATE TABLE IF NOT EXISTS " + getTicketCommentsTableName() + " ("
                        + "`id` INT( 16 ) NOT NULL AUTO_INCREMENT ," + "`ticket_id` INT( 16 ) NOT NULL DEFAULT '0',"
                        + "`commenter` VARCHAR( 16 ) NOT NULL ," + "`comment` TEXT NOT NULL,"
                        + "`date` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, " + "PRIMARY KEY (`id`)," + "INDEX (`ticket_id`)"
                        + ") ENGINE = MYISAM;", new Object[0]);
    }
    ...
    public static boolean isAdmin(Player player) {
        return (player.hasPermission("helpticket.admin.assign") || player.hasPermission("helpticket.admin.tp")
